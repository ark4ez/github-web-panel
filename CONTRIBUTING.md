...
Each release needs the exact ZIP, SHA-256, Plugin Verifier report and a completed acceptance record in RELEASE-CHECKLIST.md. [Release CI](docs/RELEASING.md) prepares an unpublished GitHub Release draft from an approved version tag. The owner makes the final GitHub publication decision; publishing a beta triggers the [Marketplace beta upload](docs/MARKETPLACE-CI.md) after token setup. Stable Marketplace publishing is not configured. License contributions under the repository's MIT license.

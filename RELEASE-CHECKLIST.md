...
Tags, GitHub publication and Marketplace approval are separate stages. [Release CI](docs/RELEASING.md) creates an unpublished GitHub Release draft only after a version tag passes the automated gates. Manual workflow runs validate without publishing. The owner reviews acceptance and the attached artifacts before publishing the draft. Publishing a numbered beta starts the [Marketplace beta workflow](docs/MARKETPLACE-CI.md): its token must be configured, and the published assets must match the successful tag-triggered CI artifacts. Stable Marketplace uploads are not configured. An accepted upload still requires Marketplace approval.

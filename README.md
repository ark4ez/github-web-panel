...
The ZIP is under `build/distributions/`; binary compatibility reports are under `build/reports/pluginVerifier/`. `check` runs account-free URL, preference and toolbar-layout regressions. Archives use stable ordering/timestamps. Tag-triggered [release CI](docs/RELEASING.md) reuses these checks and creates an unpublished GitHub Release draft with the ZIP, checksum and verification reports. Publishing a beta GitHub Release triggers the [Marketplace beta workflow](docs/MARKETPLACE-CI.md), using the same verified ZIP once its environment token is configured. The final GitHub publication decision remains manual; Marketplace approval is separate.
